...
class GEngine(object):
    def __init__(self, fullscreen=False):
        ...
        if fullscreen:
            self._screen = self._pg.display.set_mode((self._width * scale_factor, self._height * scale_factor), self._pg.FULLSCREEN)
        else:
            self._screen = self._pg.display.set_mode((self._width * scale_factor, self._height * scale_factor))
        self._clock = self._pg.time.Clock()
    ...
    def _run(self):
        ...
        while running:
            ...
            if self._state == GState.kMenu:
                if pressed_keys[self._pg.K_RETURN]:
                    self._state = GState.kLevel
            if self._state == GState.kLevel:
                self._run_game()
            ...
            if self._state == GState.kMenu:
                self._run_menu()
            elif self._state == GState.kLevel:
                self._run_level()

            surf = pg.surfarray.make_surface(self._canvas.get_pixels())
            surf = self._pg.transform.scale(surf, (self._width * scale_factor, self._height * scale_factor))
